...
<div class="flex flex-col h-[calc(100vh-110px)] -m-4 sm:-m-8" x-data="chatApp()" x-init="loadMessages(); setInterval(() => loadMessages(), 5000);">
    <!-- Chat Header -->
    <div class="bg-white border-b border-slate-200 px-4 py-3 sm:px-6 flex items-center justify-between shrink-0 z-10">
        <div class="flex items-center gap-3 sm:gap-4 overflow-hidden">
            <a href="{{ route('dashboard.support.index') }}" class="p-2 -ml-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors">
            ...
            <div class="flex flex-col min-w-0">
                ...
                <p class="text-xs text-slate-500 flex items-center gap-1.5">
                    ...
                    <span class="sm:hidden" x-show="ticketStatus !== 'closed'">•</span>
                    <span class="sm:hidden font-medium" 
                          :class="{
                              'text-blue-600': ticketStatus === 'open',
                              'text-amber-600': ticketStatus === 'in_progress',
                              'text-emerald-600': ticketStatus === 'resolved',
                              'text-slate-600': ticketStatus === 'closed'
                          }"
                          x-text="formatStatus(ticketStatus)"></span>
                </p>
            </div>
        </div>

        <!-- Desktop Status Badge (Right Side) -->
        <div class="hidden sm:flex items-center gap-3">
            @if($ticket->assignedTo)
                <div class="flex items-center gap-2 text-xs text-slate-500 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-100">
                    ...
                    <span>Atendido por <span class="font-semibold text-slate-700">{{ $ticket->assignedTo->name }}</span></span>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white border-t border-slate-200 p-4 shrink-0 z-20">
        <template x-if="ticketStatus === 'closed'">
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col items-center text-center max-w-2xl mx-auto">
                <div class="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center mb-2 text-slate-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                    </svg>
                </div>
                <h3 class="font-bold text-slate-800">Ticket Encerrado</h3>
                <p class="text-sm text-slate-500 mt-1">Este atendimento foi finalizado. Se precisar de mais ajuda, por favor abra um novo ticket.</p>
            </div>
        </template>
...
function chatApp() {
    return {
        ...
        async sendMessage() {
            ...
            try {
                const url = '{{ route("dashboard.support.message", ["id" => 999]) }}'.replace('999', this.ticketId);
        ...
        async loadMessages(forceScroll = false) {
            try {
                const url = '{{ route("dashboard.support.messages", ["id" => 999]) }}'.replace('999', this.ticketId);
                const response = await fetch(url, {
                ...
                if (data.ticket_status && data.ticket_status !== this.ticketStatus) {
                ...
                // If no new messages and not initial load, skip rendering to prevent flicker
                ...
                const html = data.messages.map((msg, index) => {
                    ...
                    const avatar = isCurrentUser 
                        ? '' // No avatar for current user to keep it clean like WhatsApp/Telegram
                        : `
                    ...
                    return `
                        ...
                        <div class="flex ${justifyClass} mb-4 group animate-fadeIn">
                            ...
                            <div class="flex flex-col ${isCurrentUser ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[70%]">
                                ${!isCurrentUser && isAdmin ? '<span class="text-[10px] font-bold text-indigo-600 mb-1 ml-1">Suporte</span>' : ''}
